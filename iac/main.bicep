...
var uniqueSuffix = take(uniqueString(resourceGroup().id, appName), 6)
var compactAppName = toLower(replace(appName, '-', ''))
var storageAccountName = 'stg${take(compactAppName, 15)}${uniqueSuffix}'
var keyVaultName = 'kv-${take(appName, 14)}-${take(uniqueSuffix, 4)}'
...
resource storageBlobRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  ...
  properties: {
    // Storage Blob Data Contributor -roolin sisaanrakennettu GUID
...
resource keyVaultSecretsRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  ...
  scope: keyVault
  properties: {
    // Key Vault Secrets User -roolin sisaanrakennettu GUID
    roleDefinitionId: subscriptionResourceId(
      'Microsoft.Authorization/roleDefinitions',
